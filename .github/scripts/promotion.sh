...
# How many squash-merged PRs are queued. The grouped list itself is rendered by
# the shared gen-changelog.sh (same script the release notes use), so the PR
# parsing + area:* grouping lives in exactly one place.
...
BODY_FILE=$(mktemp)
{
  ...
  # The grouped PR list — same renderer the Stable/Nightly release notes use.
  bash "$(dirname "$0")/gen-changelog.sh" "origin/main..HEAD"
